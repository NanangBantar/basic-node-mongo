document.getElementById("form").addEventListener("submit", async (e) => {
    ...
    try {
        const formData = {
            nama_karyawan: document.querySelector("input[name='nama_karyawan']").value,
            alamat_ktp: document.querySelector("input[name='alamat_ktp']").value,
            nomor_hp: document.querySelector("input[name='nomor_hp']").value,
            nomor_serumah: document.querySelector("input[name='nomor_serumah']").value,
        ...
        const resp = await axios.post("http://localhost:3000/api/personaldata", formData);
